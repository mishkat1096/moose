...
#include "ACInterfaceKobayashi.h"

registerMooseObject("PhaseFieldApp", ACInterfaceKobayashi);

InputParameters
ACInterfaceKobayashi::validParams()
{
...
ACInterfaceKobayashi::ACInterfaceKobayashi(const InputParameters & parameters)
...
RealGradient
ACInterfaceKobayashi::precomputeQpResidual()
{
...
RealGradient
ACInterfaceKobayashi::precomputeQpJacobian()
{
  // Set modified gradient vector
  const RealGradient v(_grad_u[_qp](1),- _grad_u[_qp](0), 0);
...
Real
ACInterfaceKobayashi::computeQpOffDiagJacobian(unsigned int jvar)
{
