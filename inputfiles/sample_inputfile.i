...
[Variables]
  ...
  [./InitialCondition]
     ...
     x1 = 100 # X coordinate of the seed
     y1 = -400 # X coordinate of the seed
     ...
     outvalue = -1 # Liquid on the outside of th seed
     int_width = 3
    [../]
  [../]
  [./U]
...
#Here the weak form for the PDE's are solved
[Kernels]
  ...
  [./Ucoupledphi] # calculates (1+(1-k)U)/2 dphi/dt
    ...
    args = 'phi'
  [../]
  [./Anti_trapping] # calculates dphi/dt * grad(U)/abs(grad(Us))
...
# AuxVariables are usd for postprocessing
[AuxVariables]
  [./z]
     order = CONSTANT
     family = MONOMIAL
...
# AuxKernels are usde for the calculation of AuxVariables
[AuxKernels]
  [./z]
...
# a function but can't depednd on on coordinate axis)
[Functions]
  [./y]
    type = ParsedFunction
    value = 'y'
...
# Here material constant and properties are calculated
[Materials]
  [./consts]
    type = GenericConstantMaterial
    prop_names  = 'Dl a1 a2 e G d pv m ci ke A'
    prop_values = '15 .8839 .6267 60 .4 .013 .01 4 2.7 0.3 0.05'
    # Dl - Diffusion Coeff
    # a1 and a2 are constants, these comes form asymptotic analysis. Usually, constant for every alloys.
    # e - W/d , signifies how large the interface will be. as cooling rate (G*V) gets higher e shuld be smaller.
    # G - thermal gradient, process parameter
    # d- is Diffusion length, a material constant.
...
# This block defines Boundary condition.
